...
void TerminalActivity::onEnter() {
  ...
  connected.store(usb.isConnected());
...
void TerminalActivity::pollUsb(const uint32_t now) {
  bool received = false;
  ...
  for (uint16_t count = 0; count < 256 && usb.available() > 0; ++count) {
    const int byte = usb.read();
    if (byte < 0) {
      break;
    }
...
void TerminalActivity::loop() {
  ...
  const bool linkConnected = usb.isConnected();
  if (connected.exchange(linkConnected) != linkConnected) {
...
void TerminalActivity::drawStatus() {
  char status[80];
  std::snprintf(status, sizeof(status), "%s - USB %s", tr(STR_KNIETTY),
                connected.load(std::memory_order_acquire) ? tr(STR_USB_CONNECTED) : tr(STR_USB_DISCONNECTED));
...
void TerminalActivity::render(RenderLock&&) {
  ...
  const bool full = forceFullRefresh.exchange(false, std::memory_order_acq_rel) || firstRender ||
...
bool TerminalActivity::preventAutoSleep() {
  return connected.load(std::memory_order_acquire) || hasQueuedBytes() || millis() - lastTrafficAt.load() < 30000;
}
